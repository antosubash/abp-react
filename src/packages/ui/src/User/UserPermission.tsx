...
import { IdentityUserUpdateDto, PermissionGroupDto, PermissionGrantInfoDto, PermissionsService, UpdatePermissionsDto} from "@abpreact/proxy";
import { v4 } from 'uuid';
...
export const UserPermission = ({userDto, onDismiss}: UserPermissionProps) => {
    ...
    useEffect(() => {
       const hasAllPermissionGranted = permissionGroups.map(g => g.permissions?.every(p => p.isGranted)).every(v => v);
       setHasAllGranted(hasAllPermissionGranted);
    }, [permissionGroups]);

    const switchManagement = useCallback((index: number) => {
        if(permissionGroups) {
            const allPermissionSelected = permissionGroups.map(g => g.permissions?.every(p => p.isGranted)).every(v => v);
    ...
    const onSubmit = useCallback(async (e: FormEvent) => {
        e.preventDefault();
        const payload = permissionGroups?.map(p => p!.permissions!.map(grant => ({name: grant.name, isGranted: grant.isGranted}))).flat();
        const requestPayload: UpdatePermissionsDto = { permissions: payload }
        try {
          await PermissionsService.permissionsUpdate(PermissionProvider.NAME, PermissionProvider.KEY, requestPayload);
            toast({
                title: "Success",
                        ...
                        <section className="flex pt-5 flex-col">
                            <section className="flex flex-col">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 justify-center content-center">
                                    {renderItems?.map(((permission: PermissionGroupDto, idx: number) => (
